...
def initialize_clients():
    """Initialize all clients with credentials from Secret Manager"""
    try:
        ...
        if not project_id:
            raise EnvironmentError("PROJECT_ID environment variable not set")

        # Load all required secrets
        secrets = {
            "DOC_AI_CREDENTIALS": None,
        ...
        for secret_name in secrets.keys():
            ...
            try:
                ...
                raise EnvironmentError(f"Failed to load secret {secret_name}: {str(e)}")

        # Initialize Firestore
        firestore_credentials_json = json.loads(os.environ["FIRESTORE_CREDENTIALS"])
        firestore_credentials = service_account.Credentials.from_service_account_info(firestore_credentials_json)
        global db
        db = firestore.Client(project=project_id, credentials=firestore_credentials)

        # Initialize Storage
        storage_credentials_json = json.loads(os.environ["STORAGE_CREDENTIALS"])
        storage_credentials = service_account.Credentials.from_service_account_info(storage_credentials_json)
        global storage_client, bucket
        storage_client = storage.Client(credentials=storage_credentials, project=project_id)
        bucket = storage_client.bucket(os.environ["BUCKET_NAME"])

        # Initialize Document AI
        doc_ai_credentials_json = json.loads(os.environ["DOC_AI_CREDENTIALS"])
        doc_ai_credentials = service_account.Credentials.from_service_account_info(doc_ai_credentials_json)
        global docai_client, PROCESSOR_NAME
        docai_client = documentai.DocumentProcessorServiceClient(credentials=doc_ai_credentials)
        PROCESSOR_NAME = f"projects/{project_id}/locations/us/processors/{os.environ['PROCESSOR_ID']}"

        # Initialize Gemini (using the latest SDK)
...
# Initialize all clients before running the app
initialize_clients()
...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        ...
        # GCS path reference
        gcs_path = f"gs://{os.environ['BUCKET_NAME']}/{username}/{thread_name}/{filename}"
