...
1.  **Incoming Section:** It can be at the top, bottom, or anywhere.
...
Note : This is just a guideline for you to know the main definition of each section. The actual components can vary and you need to identify them based on the image and OCR data. Your main task is to go from top to bottom and identify the components and their relationships.

**Instructions:**

1.  Diagram Analysis: Use the OCR data (raw_text, bounding_boxes, component_count, detailed_components) to identify components and group them into incoming, bus bar, and outgoing sections. Prioritize the understanding of component types and relationships from the image rather than strictly following a top-down approach. Use spatial relationships where available but don't rely just on the spatial positions.
...
3.  How to use the OCR data:
    * OCR data has 4 paramters : 1) raw_text 2) bounding_boxes 3) component_count 4) detailed_components
    * raw_text : Contains the text extracted from the image.
    * bounding_boxes : Contains the bounding boxes of the text extracted from the image. Use this to group nearby texts together for component classification. Go co-ordinates wise to group nearby texts together.
    * component_count : Contains the count of each component identified in the image. Use this to identify the components and set the quantities.
    * detailed_components : Contains the detailed information about each line of text extracted from the image. Use this to identify the components and set the quantities.

4.  **HTML Table Structure:** Construct an HTML table to represent this structure.
    *   The table should have 5 columns: "Serial Number", "Section", "Name", "Quantity", and "Specifications".
     *   Use three sections for table rows: "Incoming Section," "Bus Bar Section," and "Outgoing Section."

5.  **Additional Notes:** If any component, specification, or quantity cannot be clearly identified and categorized into one of the three sections, include the component or the un-parsed data in a separate section called "Additional Notes" below the table output in the text format. Use simple bullet points for clarity. For instance use - before every point. If no component needs to be included in additional notes, don't include it.

6.  **Output:** Return the data in the format of the HTML Table. Followed by the Additional Notes if any is there.

        <div style='margin-bottom: 8px;'>
        <table style='border-collapse: collapse; width: 100%;'>
            <thead>
                <tr>
                    <th style='border: 1px solid black; padding: 8px;'>Serial Number</th>
    ...
    Don’t leave anything, consider everything!"""
